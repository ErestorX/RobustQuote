...
class VisionTransformer(nn.Module):
    ...
    def forward_from_block2(self, x):
        x = self.blocks[1:](x)
        x = self.norm(x)
...
def deit_base_patch16_224(pretrained=False, patch_size=16, args=None, **kwargs):
    """ DeiT-base model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-21k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('deit_base_patch16_224', pretrained=pretrained,args=args, **model_kwargs)
    return model